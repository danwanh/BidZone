...
export const createQuestion = async (req, res) => {
    try{
        ...
        if(!mongoose.Types.ObjectId.isValid(product_id)){
            ...
            return res.status(400).json({ message: "Invalid seller_id ObjectId format" });
        }else if(!mongoose.Types.ObjectId.isValid(bidder_id)){
            return res.status(400).json({ message: "Invalid bidder_id ObjectId format" });
        }

        if (!product_id || !seller_id || !bidder_id || !question){
            return res.status(400).json({ message: "Missing required fields" });
        }else if (seller_id === bidder_id){
            return res.status(400).json({ message: "User can't ask/answer themself "});
        }
...
export const updateQuestion = async (req, res) => {
    ...
    try{
        ...
        const updated = await Question.findByIdAndUpdate(req.params.id, res.question, {new: true});
        res.json(updated);
    }catch(err){
