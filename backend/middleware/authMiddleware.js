    ...
    return async (req, res, next) => {
        try {
            ...
            if (resourceType === "order") {
                ...
                const isBuyer = resource.buyer_id.toString() === req.user._id.toString();
                const isSeller = resource.seller_id.toString() === req.user._id.toString();
...
export const validateOrderData = (req, res, next) => {
    ...
    if (!product_id || !seller_id || !buyer_id || !address) {
        return res.status(400).json({ 
            message: "Missing required fields: product_id, seller_id, buyer_id, address" 
        });
    }
