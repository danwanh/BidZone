...
const ProfileProductList = ({ baseURL, xtra, user }) => {
  ...
  return (
    <section className="h-full flex flex-col min-h-[600px]">
      ...
      {error && <div className="text-red-500 p-4">Error loading products: {error}</div>}
      {!loading && !error && (
        <>
          {products.length > 0 ? (
            <div className="flex flex-col items-center gap-6 sm:gap-8 flex-1">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 w-full auto-rows-fr">
                {products.map((p) =>
                  p.product_id?._id ? (
            ...
            <div className="w-full text-center py-8 text-lg font-semibold text-gray-500">Danh sách đang trống</div>
          )}
        </>
      )}
    </section>
