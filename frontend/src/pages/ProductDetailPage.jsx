...
export const ProductDetailPage = () => {
  ...
  const handleBid = useCallback(async () => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.message || error.response?.data?.error || "Không thể đặt giá"
  ...
  const handleAppendDescription = useCallback(async () => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.message || "Không thể cập nhật mô tả"
      alert(errorMsg)
    }
  }, [newDescription, id])
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      <SellerInfo seller={seller} highestBidder={highestBidder} currentBid={currentBid} />
